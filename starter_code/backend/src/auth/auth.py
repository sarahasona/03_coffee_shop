...
AUTH0_DOMAIN = 'dev-mv6js4zd.us.auth0.com'
ALGORITHMS = ['RS256']
API_AUDIENCE = 'CoffeShopDrinks'
...
'''
@TO_DO implement get_token_auth_header() method
...
def get_token_auth_header():
    ...
    return token
'''
@TO_DO implement check_permissions(permission, payload) method
    @INPUTS
...
def check_permissions(permission, payload):
    if 'permissions' not in payload:
        raise AuthError({"code": "invalid_header",
                        "description":
                            "token do not contain permessions"}, 400)
    if permission not in payload['permissions']:
        raise AuthError({"code": "No_Permission",
                        "description":
                            "not allowed to access"}, 403)
    return True
'''
@TO_DO implement verify_decode_jwt(token) method
    @INPUTS
...
'''
@TO_DO implement @requires_auth(permission) decorator method
    @INPUTS
        permission: string permission (i.e. 'post:drink')
